...
-   **原理与改进**:
    此模式结合了GARCH模型的时变波动率特性和引力GBM的价格约束机制。**GARCH (广义自回归条件异方差)** 模型专门用于捕捉金融时间序列中**"波动率聚集"**的特性——即市场在一段时期内的高波动之后往往伴随着高波动，平静之后则继续平静。
    
    脚本首先用GARCH(1,1)模型拟合历史收益率数据，得到一个随时间动态变化的条件波动率序列。然后采用与引力GBM相同的三项式结构生成新价格：
    
    **新价格 = 前期价格 + 漂移项 + 扩散项 + 引力项**
    
    其中：
    - **漂移项**: `μ × 前期价格 × drift_scale` - 基于历史收益率均值的趋势项
    - **扩散项**: `GARCH波动率[t] × 前期价格 × 随机冲击` - 使用时变的GARCH条件波动率
    - **引力项**: `G × (原始价格[t] - 前期生成价格)` - 将生成价格拉向原始历史价格

-   **参数 (`CUSTOM_PARAMETERS['GARCH']`)**:
    -   `sigma_scale` (float): 应用于GARCH模型预测的条件波动率的缩放因子。它允许你全局性地放大或缩小市场的整体波动水平。
    -   `G` (float): 引力系数。控制生成价格向原始价格的回归强度。`1.0`表示完全跟随原始价格路径，`0.0`表示纯GARCH随机游走，`0.5`表示混合模式。
    -   `drift_scale` (float): 漂移率缩放系数。控制历史趋势在新价格中的影响程度。

-  **特点**:
    - 现代，成熟，应用广泛的建模方法
    - 结合了GARCH的时变波动率和引力约束的价格合理性
    - 相比纯GARCH模型，生成的价格路径更贴近"可能发生但未发生"的合理范围

## 4. 趋势-残差分解模式 (`Trend_Residual`)

-   **原理与改进**:
    此模式采用全新的价格分解思路，将对数价格序列分解为**趋势成分**和**残差成分**，然后分别建模和重构。这种方法避免了引力机制的依赖，能够生成更加独立和多样化的价格路径。
    
    **核心分解公式**: `log(P_t) = Trend_t + Residual_t`
    
    **三步生成流程**:
    1. **趋势提取**: 使用单侧指数加权移动平均(EWM)从历史对数收盘价中提取趋势成分，确保只使用过去信息
    2. **残差建模**: 对残差增量序列应用GARCH(1,1)模型，获得时变的条件波动率
    3. **价格重构**: 按公式 `log(P*_t) = trend_scale × Trend_t + Residual*_t` 重新组合
    
    **OHLC重构机制**:
    - **开盘价**: 遵循加密货币24/7交易特性，设定 `open_t = close_{t-1}`（无跳空）
    - **高低价**: 基于原始数据的影线比例，加入适量噪声后重构上下影线
    - **约束强制**: 确保 `high ≥ max(open, close)` 和 `low ≤ min(open, close)`

-   **参数 (`CUSTOM_PARAMETERS['Trend_Residual']`)**:
    -   `trend_halflife` (int): 趋势平滑的半衰期（小时）。值越大，趋势越平滑，对短期波动的敏感度越低。默认24小时。
    -   `trend_scale` (float): 趋势强度缩放系数。`1.0`保持原始趋势强度，`<1.0`弱化趋势，`>1.0`强化趋势。
    -   `sigma_scale` (float): 残差波动缩放系数。控制生成价格的整体波动水平。
    -   `sigma_max` (float): 残差波动率上限（对数尺度，每小时）。防止极端波动，默认无限制。
    -   `wick_scale` (float): 影线比例缩放系数。控制高低价相对于开收盘价的延伸程度。
    -   `wick_noise_std` (float): 影线噪声标准差。为影线长度添加随机变动，增加真实感。

-  **特点**:
    - **独立性强**: 不依赖引力机制，生成的价格路径更加独立和多样化
    - **趋势保持**: 通过趋势缩放参数，可以灵活控制宏观价格方向
    - **波动率真实**: 使用GARCH建模残差，保持了波动率聚集的金融特性
    - **微观结构完整**: 精细重构OHLC关系，保持K线图的视觉真实性
    - **参数丰富**: 提供多个维度的控制参数，适应不同的回测需求
